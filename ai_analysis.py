...
def _get_config():
    try:
        ...
        raise RuntimeError('config.py not found. Copy config.example.py to config.py and set your API key.')
    if not api_key or api_key == 'YOUR_KEY_HERE':
        raise RuntimeError('OpenAI API key is not configured. Set OPENAI_API_KEY in config.py.')
